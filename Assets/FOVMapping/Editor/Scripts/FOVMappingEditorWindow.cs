...
public class FOVMappingEditorWindow : EditorWindow
{
    ...
    [MenuItem("Window/FOV Mapping/Bake an FOV Map")]
    ...
    private void OnGUI()
    {
        EditorGUILayout.BeginVertical(new GUIStyle(GUI.skin.window));
        {
            ...
            if (fovManager == null)
            {
                ...
                {
                    EditorGUILayout.HelpBox($"Multiple FOVManagers found in scene ({managers.Length}). Please select only one.", MessageType.Error);
                }
            }
            
            fovManager = EditorGUILayout.ObjectField("FOV Manager (Plane GameObject)", fovManager, typeof(FOVManager), true) as FOVManager;
